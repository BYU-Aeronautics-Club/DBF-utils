...
%		\\
		% Second row
		&
		\node [Inputs] (Input_1) {Mission Requirements};
		\\
		\\
		% Third row
		\node [CAD] (Output_0) {Final Aircraft}; 
		& 
		\node [Design,stack,text width=2.3cm] (Design) {Design Iterations}; 
		&
		\node [CAD] (Concept_CAD) {Concept CAD}; 
		&
		\node [CAD] (Preliminary_CAD) {Balsa Wing \& Composite Fuselage}; 
		&
...
%		\node [Prototype] (Output_1) {Final Aircraft}; 
		& 
		\node [Testing] (Concept_test) {Improved CAD Modeling}; 
		& 
		\node [Prototype,text width=1.8cm] (Concept_prototype) {Foam Hot-wiring Prototype};
		\\
		% 5th row
		& 
		\node (Preliminary_test) {}; 
		& 
		&
		\node [Prototype,text width=1.8cm] (Preliminary_prototype) {Assembly \& Testing};
		\\
		% 6th row
%		& 
...
%			   (Preliminary_prototype) edge (Preliminary_test)
			   (Preliminary_prototype) -| (Concept_test);
%	\draw [-{Latex[length=1.5mm]},draw=\BYUblue] 
%			   (Detailed_CAD) edge (Detailed_prototype) 
%			   (Detailed_prototype) edge (Detailed_test);
